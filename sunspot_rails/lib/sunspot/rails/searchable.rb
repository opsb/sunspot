    ...
    module Searchable
      ...
      module ClassMethods
        ...
        def solr_index(opts={})
          ...
          solr_index_scope = self.unscoped.includes(options[:include])

          if options[:batch_size].to_i > 0
            batch_counter = 0
            solr_index_scope.includes(options[:include]).find_in_batches(options.slice(:batch_size, :start)) do |records|
            ...
            Sunspot.index! solr_index_scope.select(&:indexable?)
          end
